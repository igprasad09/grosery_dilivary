...
function Addtocart() {
    ...
    useEffect(() => {
        axios.get("https://grosery-dilivary.vercel.app/api/v1/addcartproducts", {
            ...
            setAddedCart(res.data.cart.userCart);
        })
    }, [removeItemFromCart]);

    useEffect(()=>{
            const p = addedCart.map((element)=>{
                      const value = parseInt(element.price);
                      return value * element.qty
            });
            const total = p.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
            setPrice(total)
            const totalwithtax = total * 1.02;
            const totaltax = totalwithtax - total;
            setTax(totaltax.toFixed(2));
            setAfterTax((totaltax+total).toFixed(2));
            setValue(addedCart.length);
    },[addedCart]);
        ...
        <div className="">
            ...
            <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
                <div className='flex-1 max-w-4xl'>
                    ...
                    {addedCart.map((product, index) => (
                        <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                            <div className="flex items-center md:gap-6 gap-3">
                                <div className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                                    <img className="max-w-full h-full object-cover" src={product.img} alt={product.product_name} />
                                </div>
                                <div>
                            ...
                            <button onClick={()=>removeItemFromCart(product)} className="cursor-pointer mx-auto">
                ...
                <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="text-gray-500 mt-4 space-y-2">
                        ...
                        <p className="flex justify-between text-lg font-medium mt-3">
                            <span>Total Amount:</span><span>${afterTax}</span>
                        </p>
                    </div>
